...
public class SoundPlayer : AbstractMonoPoolable
{
    ...
    public void PlaySound(SoundClipSO clipData, Transform trans = null)
    {
        ...
        {
            Debug.LogWarning("Cannot play sound because SoundClipSO or AudioClip is missing.", this);
        ...
        if (!clipData.loop)
        {
            float time = _audioSource.clip.length / Mathf.Abs(_audioSource.pitch) + .2f;
