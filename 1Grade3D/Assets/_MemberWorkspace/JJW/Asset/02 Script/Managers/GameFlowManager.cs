...
namespace _MemberWorkspace.JJW.Asset._02_Script.Managers
{
    ...
    public class GameFlowManager : MonoBehaviour
    {
        ...
        {
            flowEventChannel?.AddListener<StormEndEvent>(HandleStormEnd);
        }

        private void OnDisable()
        {
            flowEventChannel?.RemoveListener<StormEndEvent>(HandleStormEnd);
        }
        ...
        public void EndTurn() //타이머 안끝나도 상점갈수 있으니까 그냥 퍼블릭으로 둠 ㅇㅇ 상점 갈때 호출
        {
            ...
            turnEventChannel.RaiseEvent(TurnEvents.TurnEndEvent);
        }

        public void CloseShop()//상점에서 아이템 고르면 실행
        {
            flowEventChannel.RaiseEvent(FlowEvents.ShopCloseEvent);
            StartStorm();
        }

        public void OpenShop()//정산 완료 버트 누를때
        {
            flowEventChannel.RaiseEvent(FlowEvents.ShopOpenEvent);
        }

        private void StartStorm()
        {
            CurrentState = GameState.Storm;
            flowEventChannel.RaiseEvent(FlowEvents.StormPrepareEvent);
        }
        ...
        private void HandleStormEnd(StormEndEvent @event)
        {
            CompleteStorm();
        }

        private void CompleteStorm()
        {
        ...
        private void StartTurn()
        {
            ...
            turnEventChannel.RaiseEvent(TurnEvents.TurnStartEvent);
        }
    }
}
