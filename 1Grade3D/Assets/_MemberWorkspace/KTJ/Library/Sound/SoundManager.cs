...
public class SoundManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("SoundChannel is not assigned.", this);
            enabled = false;
            return;
        }
    ...
    {
        if (evt == null || evt.ClipData == null || evt.ClipData.clip == null)
        {
            Debug.LogWarning("PlaySoundEvent has no playable clip.", this);
            return;
        }
        ...
        if (player == null)
        {
            Debug.LogError("Failed to get a SoundPlayer from the pool.", this);
            return;
        }
    ...
    private void HandleSoundFinish(SoundPlayer player)
    {
        ...
        int channelToRemove = 0;
        foreach (KeyValuePair<int, SoundPlayer> pair in _soundPlayerDict)
        {
            if (pair.Value == player)
            {
                channelToRemove = pair.Key;
                break;
            }
        }

        if (channelToRemove > 0)
            _soundPlayerDict.Remove(channelToRemove);
